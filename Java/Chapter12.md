...
- 두 generic class가 상속관계이고, 대입된 타입이 일치하는 것은 가능하다.
...
- 인터페이스의 경우, implements가 아닌 extends를 사용한다.
```java
import java.util.ArrayList;
...
##### 1.5 와일드 카드 '?'
- generic 타입에 와이들 카드를 쓰면, 여러 타입의 대입이 가능하다.
...
- 클래스 타입 매개변수 <T>와 메서드 타입 매개변수 <T>는 별개의 매개변수이다.
...
##### 2.1 열거형의 정의와 사용
열거형의 정의 : enum 열거형이름 { 상수명1, 상수명2, ... }
아래의 코드와 같이 열거형 타입의 변수를 선언하고 사용할 수 있으면 ==와 compareTo( )로 비교 가능하다.
```java
